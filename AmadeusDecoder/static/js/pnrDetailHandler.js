const ServiceFeesInput = document.querySelectorAll('.inputeditable.montant.fee-cost');
...
const confirmationCustomerCheckbox = document.querySelectorAll('.confirmation-customer-checkbox');
const totalAmountPerCheckboxChecked = document.getElementById("totalAmountPerCheckboxChecked");
const spanTotalAmountPerCheckboxChecked = document.getElementById("spanTotalAmountPerCheckboxChecked");
const totalAmountOrder = totalAmountPerCheckboxChecked.getAttribute("data-total-amount-order");
const EachAmount = JSON.parse(totalAmountPerCheckboxChecked.getAttribute("data-each-amount-order")) || [];
...
const resetAmountTotalValue = () => {
    confirmationCustomerCheckbox.forEach((checkBox) => {
        ...
        if (!checkBox.disabled) {
            if (!countDeDuplicatedCustomer.includes(checkBox.getAttribute("data-customer-id"))) {
                countDeDuplicatedCustomer.push(checkBox.getAttribute("data-customer-id"));
            }
            ...
            if (checkBox.checked && !checkBox.disabled) {
                ...
                localStorage.setItem("listCustomerIdsAfterChecked", JSON.stringify(listCustomerIds));
            ...
            checkBox.disabled ? confirmationCustomerCheckboxChecked-- : confirmationCustomerCheckboxChecked + 0;
        }
    });
}
...
confirmationCustomerCheckbox.forEach(checkBox => {
    checkBox.addEventListener("change", (e) => {
        if (checkBox.checked && !checkBox.disabled) {
            ...
            let NumberOfElementToRemove = countDeDuplicatedCustomer.length - confirmationCustomerCheckboxChecked

            listCustomerIds.splice(index, NumberOfElementToRemove)
    
            localStorage.setItem("listCustomerIdsAfterChecked", JSON.stringify(listCustomerIdsAfterChecked))
        ...
        function totalAmountFromLocalStorage () {
            let listCustomerIdsAfterCheckedLocalStorage = JSON.parse(localStorage.getItem("listCustomerIdsAfterChecked"));

            let amountTotalAfterChecked = [];

            listCustomerIdsAfterCheckedLocalStorage.forEach((customerId) => {
                const customerData = EachAmount.filter(item => item.customer_id == customerId);
            ...
            const totalAmountAfterReload = amountTotalAfterChecked.reduce((sum, item) => sum + item.total, 0);

            spanTotalAmountPerCheckboxChecked.textContent = totalAmountAfterReload.toFixed(2);
        }
    })
})
...
CreateOrder.addEventListener('click', (e)=> {
    ...
    document.querySelectorAll('.confirmation-customer-checkbox').forEach(checkbox => {
        if (checkbox.checked && !(checkbox.disabled)) {
...
$(document).ready(function () {
    ...
    $("#create-command").click(function () {
        if (customerDefault == null || customerDefault.textContent == "") {
            ...
            toastr.error("Veuillez selectionner un client.");
        } else if (count__ticketCheckBoxPassenger > 0 && totalAmount == 0 || count__otherFeeCheckBox > 0 && totalAmount == 0 || count__feeCheckBox > 0 && totalAmount == 0) {
            toastr.info("Veuillez devez sélectionner un nouveau client pour les nouveaux billets.");
            setTimeout(() => {
                removeModalOrderConfirmAttribute()
...
const allCheckBox     = document.querySelectorAll("#passagers-check.passengers-align-checkboxes");
let allCheckBoxLength = document.querySelectorAll("#passagers-check.passengers-align-checkboxes").length;
...
let ticketCheckBoxPassenger = document.querySelectorAll('.ticket-checkbox-passenger');
...
ticketCheckBoxPassenger.forEach((checkBox) => {
    checkBox.hidden ? count__ticketCheckBoxPassenger -- : count__ticketCheckBoxPassenger + 0
})
...
const ProductDropdown = document.querySelector('[name="service-type-dropdown"]');
const ProductDesignaInput = document.querySelector('[name="design-input-line"]');
const ProductTranspInput = document.querySelector('[name="transport-input-line"]');
const ProductTaxInput = document.querySelector('[name="taxe-input-line"]');
const ProductpassInput = document.querySelector('[name="passenger-input-line"]');
...
document.getElementById('save-product-select').addEventListener('click', (e) => {
    SaveProductCounting++;
    let listNewProduct = [];
    const designation = ProductDropdown.children[ProductDropdown.selectedIndex].getAttribute('data-designation');
    document.querySelector('.tr-add-line').hidden = false;
    document.getElementById('add-product-service-line').hidden = false;
    listNewProduct.push(ProductDropdown.value, ProductTypeInitiale.textContent, designation, parseFloat(ProductTranspInput.value).toFixed(2), parseFloat(ProductTaxInput.value).toFixed(2), (parseFloat(ProductTranspInput.value) + parseFloat(ProductTaxInput.value)).toFixed(2), ProductpassInput.value, '')
...
allCheckBox.forEach((checkBox) => {
    checkBox.addEventListener("change", () => {
        ...
        pnrDetailSave.addEventListener("click", (e) => {            
            if (customerListSelection.value == null || customerListSelection.value == "" && feeCountChecking < 1) {             
                e.preventDefault()
                pnrDetailSave.setAttribute("data-target", "#alertModalEmptyCustomer");
            } else if (checkBoxChecked == 0 && feeCountChecking < 1) {
                e.preventDefault()
                pnrDetailSave.setAttribute("data-target", "#alertModalNoPassengerSelected");
            } else {
                pnrDetailSave.removeAttribute("data-target");
            }
        })
...
document.getElementById("save").addEventListener("click", (e) => {
    ...
    document.querySelectorAll('.ticket-checkbox-passenger').forEach(input => {
        if (input.getAttribute('data-ticket-status') == 0 && input.checked && !input.disabled && !input.hidden) {
            listTicketCheckboxesAddAfterOrderAlreadyCreated.push(input.getAttribute('data-ticket-id'));
        }
    })
    ...
    const listPassengersCheckboxes = document.querySelectorAll('.passengers-align-checkboxes-first');
    listPassengersCheckboxes.forEach(passengersCheckbox => {
        if (passengersCheckbox.checked && !passengersCheckbox.disabled && !passengersCheckbox.hidden) {
    ...
    const listOtherFeesCheckboxes = document.querySelectorAll('.other-fees-checkboxes');
    listOtherFeesCheckboxes.forEach(otherFeesChecked => {
        if (otherFeesChecked.checked && !otherFeesChecked.disabled && !otherFeesChecked.hidden){
            listotherFeesChecked.push(otherFeesChecked.getAttribute('data-other-fees-id'));
        }
    });

    if (checkBoxChecked < 1 && CheckboxHiddenCount == checkBoxChecked && feeCountChecking < 1) {
        if (customerListSelection.value.trim().length == 0) {
            // toastr.error('Aucune action efféctuée.');
            window.location.reload();
        }
    }
    if (checkBoxChecked > 0 || feeCountChecking > 0 || count > 0 || customerListSelection.value.trim().length > 0) {
        ...
        $.ajax({
            ...
            data: {
                ...
                pnrId: pnrIdNew,
                refCde: RfeCde.value,
                ticketIdsChecked: JSON.stringify(listTicketCheckboxesAddAfterOrderAlreadyCreated),
...
document.querySelectorAll('.delete-other-fee-row').forEach(button=> {
    button.addEventListener('click', e=> {
        ...
        console.log(button.parentElement.parentElement.getAttribute('data-other-fee-id'));
    })
})
...
const removeCustomerButton = document.querySelectorAll('.remove-customer-button');
const ModalRemoveCustomer = document.getElementById('ModalRemoveCustomer');
const ModalRemoveCustomerBody = document.getElementById('ModalRemoveCustomerBody');

removeCustomerButton.forEach(button => {
    button.addEventListener('click', ()=> {
        const customerIdOnRemoveModal = ModalRemoveCustomer.getAttribute('data-customer-id');
        if (customerIdOnRemoveModal == null || customerIdOnRemoveModal == '') {
            ModalRemoveCustomerBody.setAttribute('data-customer-id', button.getAttribute('data-customer-id'));
        };
        ModalRemoveCustomerBody.textContent = `Les commandes pour le client "${button.getAttribute('data-customer-intitule')}" seront  supprimées.`;
    })
})
...
const CheckboxForTotalRecalculation = document.querySelectorAll('.checkbox-line-below');
const CheckboxOfPassenger = document.querySelectorAll('.passengers-align-checkboxes');
...
CheckboxForTotalRecalculation.forEach(checkbox => {
    checkbox.addEventListener('change', () => {
        ...
        if (!checkbox.hidden && checkbox.checked) {
            const otherCheckbox = document.querySelectorAll(`input[data-commune-id="${communeId}"]`);
 ...
 */
const modal__addNewClientForOtherPassenger = document.getElementById("addNewClientForOtherPassenger")
const button__createNewClientForOtherPassenger = document.getElementById("createNewClientForOtherPassenger")
const button__confirmSelectionCustomer = document.getElementById("confirmSelectionCustomer")
let select__customersList = document.getElementById("customer-list");
if (button__createCustomer != null) {
    if (button__createCustomer.getAttribute("data-passenger-informations") != "None") {
        removeModalAttribute();
        let firstPassengerInformations = JSON.parse(button__createCustomer.getAttribute("data-passenger-informations"));
        ...
        if (name != null && surName != null) {
            valueToFind = [firstPassengerInformations.name.trim(), firstPassengerInformations.surname.trim()]  
        }
...
const input__confirmationCustomerOrderCheckbox = document.querySelectorAll("#ConfirmationCustomerOrderCheckbox")
const jsonParseTotalAmount = JSON.parse(totalAmountPerCheckboxChecked.getAttribute("data-each-amount-order"))
...
//     })
// }
const span__confirmationCustomerDefault = document.querySelectorAll(".confirmation-customer-default")
